    ...
    for numero_cm, items_del_cm in grupos_cm.items():
        ...
        for item_num in items_del_cm:
            ...
            for _, subrow in sub.iterrows():
                ...
                # Buscar ítem correspondiente en el CM por código de parte
                item_cm = None
                for ic in items_cm:
                    codigo_cm = normalizar_codigo(ic.get("codigo_parte", ""))
                    if codigo_cm == modelo_di:
                        item_cm = ic
                        break

                if not item_cm:
                    # Intentar buscar por NCM
                    for ic in items_cm:
                        ncm_cm_8 = ic.get("ncm_8_digitos", "").replace(".", "")[:8]
                        if ncm_cm_8 == ncm_di_8:
                            item_cm = ic
                            break
                ...
                if not item_cm:
                    resultados.append(alerta(
                        item_num, "CM",
                        f"No se encontró en el CM el código de parte '{modelo_di}' ni el NCM '{ncm_di_8}'",
                        "ERROR"
                    ))
                    continue
                ...
                if ncm_di_8 != ncm_cm_8:
                    resultados.append(alerta(
                        item_num, "NCM",
                        f"NCM DI: {ncm_di_8} — NCM CM: {ncm_cm_8}",
                        "ERROR"
                    ))
                else:
                ...
                if modelo_di != codigo_cm:
                    resultados.append(alerta(
                        item_num, "MODELO",
                        f"Código DI: '{modelo_di}' — Código CM: '{codigo_cm}'",
                        "ERROR"
                    ))
                else:
                ...
                if cantidad_di > cantidad_cm:
                    resultados.append(alerta(
                        item_num, "CANTIDAD",
                        f"Cantidad DI ({cantidad_di}) supera la habilitada en CM ({cantidad_cm})",
                        "ERROR"
                    ))
                else:
                ...
                if abs(fob_di - fob_cm) > TOLERANCIA_FOB:
                    resultados.append(alerta(
                        item_num, "MONTO FOB",
                        f"FOB DI: {fob_di:.2f} — FOB CM: {fob_cm:.2f} (diferencia: {abs(fob_di - fob_cm):.2f})",
                        "ALERTA"
                    ))
                else:
